...
class NewtonCG(optim.Optimizer):
    ...
    def __init__(self, params, lr=1.0, rank=10, mu=1e-4, chunk_size=1,
        ...
        if len(self.param_groups) > 1:
            raise ValueError(
                "NewtonCG doesn't currently support per-parameter options (parameter groups)")

        if self.line_search_fn is not None and self.line_search_fn != 'armijo':
            raise ValueError("NewtonCG only supports Armijo line search")
    ...
    def step(self, closure=None):
        ...
        for group_idx, group in enumerate(self.param_groups):
            ...
            if self.line_search_fn == 'armijo':
                ...
                def obj_func(x, t, dx):
                    self._add_grad(t, dx)
                    loss, _ = closure()  # Need to call closure to get updated loss
                    self._set_param(x)
                    return loss
