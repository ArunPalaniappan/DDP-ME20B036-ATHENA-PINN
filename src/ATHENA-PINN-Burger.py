...
def sample_points_exp_res_grad_hess(model, t_x_matrix, num_samples):
    ...
    grads = torch.autograd.grad(u, Z, torch.ones_like(u), create_graph=True)[0]
    u_t = grads[:, 0:1]
    u_x = grads[:, 1:2]
    
    u_xx = torch.autograd.grad(u_x, Z, torch.ones_like(u_x), create_graph=True)[0][:, 1:2]
    
    f = (u_t + u * u_x - viscosity * u_xx).view(-1)
...
x_min, x_max = -1, 1
t_min, t_max = 0, 1
h = (x_max - x_min) / (Nx - 1)  # Spatial step
k = (t_max - t_min) / Nt  # Time step
...
im2 = plt.imshow(u_pred, extent=[-1, 1, 0, 1], origin="lower", aspect="auto", cmap="jet")
